# Hi, I'm Zian Fahrudi 👋

👨‍💻 Full-time **Flutter Developer at Goldstep Indonesia**  
---

## 🚀 About Me
- 📱 5+ years experience building **high-performance Flutter apps (Android & iOS)**
- 🛠️ Using **FlutterFlow** to accelerate development with no-code power
- 🐸 Building scalable backend using **Dart Frog**
- 🎯 Focused on creating **useful, real-world solutions**
---

## 🧩 What I'm Building

### 🧠 Personal Product Direction
I focus on building **micro-niche productivity apps** —  
simple, focused tools that solve specific daily problems.

### ⏰ Current Project
> An **Alarm App — but not your typical alarm.**

Designed to improve:
- waking consistency  
- habit building  
- real behavioral change  

*(More details coming soon 👀)*

---

## 🌐 Featured Projects

### 🚀 [AWundangan.com](https://awundangan.com)
Digital wedding invitation platform  
✨ Helped **50+ couples** celebrate their special day

---

### 🎨 [FlutterFlow Marketplace](https://marketplace.flutterflow.io/creator/d31d48ee5aee9ea76206566ef9e49b39893c9a52)
Creating and selling premium templates  
📦 2+ products published

---

### 📦 [pub.dev Packages](https://pub.dev/publishers/zianfahrudy.my.id/packages)
Maintaining open-source Flutter packages for the community

---

## 🌍 My Presence

[![Website](https://img.shields.io/badge/Website-000000?style=for-the-badge&logo=browser&logoColor=white)](https://tools.ziandev.site)

[![Play Store](https://img.shields.io/badge/Play_Store-000000?style=for-the-badge&logo=google-play&logoColor=white)](#)
[![App Store](https://img.shields.io/badge/App_Store-000000?style=for-the-badge&logo=apple&logoColor=white)](#)

> 🚧 Developer accounts for App Store are currently in activation process

---

## 📢 Content & Community

I share knowledge about:
- Flutter development
- Productivity tools
- Real-world app building

[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@zianfahrudi)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/zian-fahrudi)
[![Threads](https://img.shields.io/badge/Threads-000000?style=for-the-badge&logo=threads&logoColor=white)](https://www.threads.com/@zianfahrudyy)

---

⭐️ *Building simple apps that actually matter*
